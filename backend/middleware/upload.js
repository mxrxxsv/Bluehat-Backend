...
const fileFilter = (req, file, cb) => {
  try {
    ...
    if (isSuspicious) {
      const error = new Error("Suspicious file detected. File type not allowed.");
  ...
  singleUpload(req, res, (err) => {
    if (err) {
      ...
      if (err.code === 'SUSPICIOUS_FILE') {
        return res.status(400).json({
          success: false,
          message: "Suspicious file detected. File type not allowed for security reasons.",
    ...
    if (req.file) {
      try {
        ...
        if (signature) {
          const fileStart = Array.from(req.file.buffer.slice(0, signature.length));
          const signatureMatch = signature.every((byte, index) => fileStart[index] === byte);
